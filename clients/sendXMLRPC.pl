		...
		if ($ans_obj->{type} eq 'MultiAnswer') {
			...
			if ($ans_obj->{type} =~ /Matrix/) {
				...
				my $ans_str = ($ans_obj->{correct_value})
					// ($ans_obj->{correct_ans})
					;    #(correct_ans can have html formatting -- not good)				$ans_str =~ s/\[//g;
				$ans_str =~ s/\]//g;
...
sub create_pdf_output {
	...
	unless (-w $working_dir_path) {
		push @errors, "Temporary directory " . $working_dir_path . " is not writeable.";
	...
	if ($?) {
		push @errors, "Failed to rename $src_path  to " . "$dest_path in directory \n" . "$mv_out";
		print join("\n", @errors);
	}
	...
	if (my $rawexit = system $pdflatex_cmd) {
		...
		my $core   = $rawexit & 128;
		push @errors,
			"Failed to convert TeX to PDF with command $pdflatex_cmd))" . " (exit=$exit signal=$signal core=$core).";
	...
	if ($?) {
		push @errors, "Failed to rename $src_path  to " . "$final_pdf_path in directory \n" . "$mv_out1";
	}
