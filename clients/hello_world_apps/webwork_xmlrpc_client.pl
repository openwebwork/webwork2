...
if (@ARGV) {
	...
	given ($command) {
		...
		when ('listLibraries') {
			...
			eval { $result = $xmlrpc_client->xmlrpcCall($command, $input); };
			if (defined($result)) {
	...
	print STDERR "Useage: ./webwork_xmlrpc_client.pl command   [file_name]\n";
	print STDERR
		"For example: ./webwork_xmlrpc_client.pl renderProblem   <source file: e.g.  input.txt, bad_input.txt \n";
...
sub listLib {
	...
	given ($ARGS[1]) {
		...
		default {
			print "The possible arguments for listLib are:"
				. "\n\t all -- print all paths"
