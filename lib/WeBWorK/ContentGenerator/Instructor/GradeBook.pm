...
sub siblings {
	...
	my $progress     = $urlpath->newFromModule("WeBWorK::ContentGenerator::Instructor::GradeBook",  $r, 
	                                        courseID => $courseID);
...
# 		      num_of_problems $num_of_problems setName $setName";
		...
		push @scores, CGI::td(sprintf("%0.2f",$totalRight));
...
#	@studentList = @myUsers;
	...
	foreach my $studentRecord (@sortedStudentRecords) {
		...
		push @studentLinks, CGI::Tr({},CGI::td(CGI::a({-href=>$self->systemLink($userStatisticsPage,
		                                                     prams=>{effectiveUser => $studentRecord->user_id}
		                                                     )},"  $last_name, $first_name  ($user_id)" ), $self->getStudentScores($studentRecord->user_id))),;	
...
# form header here, and make appropriate modifications
	...
	if ( $setIsVersioned ) {
		print CGI::start_form({'method' => 'post', 
				       'action' => $self->systemLink($urlpath,authen=>0),'name' => 'GradeBook'});
