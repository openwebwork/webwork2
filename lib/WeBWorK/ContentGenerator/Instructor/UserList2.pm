...
#   Always have a definite sort order even if first three sorts don't determine things
	...
	print CGI::start_form({method=>"post", action=>$self->systemLink($urlpath,authen=>0), id=>"classlist-form", name=>"classlist-form", class=>"edit_form"});
...
sub output_JS{
	...
	print CGI::start_script({type=>"text/javascript", src=>"$site_url/js/legacy/tabber.js"}), CGI::end_script();
