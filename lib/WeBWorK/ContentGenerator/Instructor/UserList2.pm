...
use constant VIEW_FORMS => [qw(filter sort edit password import import2 export add delete)];
...
sub import2_form{
	...
	return join(" ",
		...
		CGI::textfield(
			-name=>'emailField',
			-value=>'Email Domain (e.x. @duq.edu)',
			-size=>30,
			-maxlength=>30
		),
		CGI::br(),
...
sub import2_handler{
	...
	#print $r->maketext("1 ".$self);
	#print $r->maketext("\n");
	#print $r->maketext("2 ".$genericParams);
	#print $r->maketext("\n");
	#print $r->maketext("3 ".$actionParams);
	#print $r->maketext("\n");
	#print $r->maketext("4 ".$tableParams);
	#print $r->maketext("\n");
	#$my $Dir1 = $self->{courseRoot};
	#my $Dir2 = $self->{pwd};
	...
	my $dir = $ce->{webworkDirs}{courses} . "/" . $courseName ."/templates/";

	my $emailExt = $self->r->param('emailField');
	unless ($fileIDhash) {
	...
	#print $r->maketext("$emailExt"." is the email Extension"."\n");
	my ($replaced, $added, $skipped)
	= $self->importUsersFromBBCSV($fileName, $createNew, $replaceExisting, @replaceList, $emailExt);
	# make new users visible... do we really want to do this? probably.
	push @{ $self->{visibleUserIDs} }, @$added;
...
sub importUsersFromCSV {
	...
	#print $r->maketext(@replaced);
	#print $r->maketext("\n");
	#print $r->maketext(@added);
	#print $r->maketext("\n");
	#print $r->maketext(@skipped);
	
	return \@replaced, \@added, \@skipped;
}

sub importUsersFromBBCSV{
...
my ($self, $fileName, $createNew, $replaceExisting, @replaceList) = @_;	 
    ...
    die $r->maketext("illegal character in input: '/'") if $fileName =~ m|/|;
    die $r->maketext("won't be able to read from file [_1]/[_2]: does it exist? is it readable?", $dir, $fileName) unless -r "$dir/$fileName";
    ...
    my (@replaced, @added, @skipped);
    # get list of hashrefs representing lines in classlist file
    my $finalFiletoPass = "$dir/$fileName";

    my @classlist = parse_BBclasslist($finalFiletoPass, @replaceList);
