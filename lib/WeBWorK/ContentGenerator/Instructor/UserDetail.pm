...
sub initialize {
	...
	$r->stash->{fields}      = DATE_FIELDS_ORDER();
	$r->stash->{fieldLabels} = DATE_FIELDS();
	...
	$self->{userRecord} = $db->getUser($editForUserID);
	return unless $self->{userRecord};
	...
	if ($r->param('save_button') || $r->param('assignAll')) {
	...
	$self->{mergedSetRecords} = { map { $_->set_id => $_ } $db->getMergedSetsWhere({ user_id => $editForUserID }) };
