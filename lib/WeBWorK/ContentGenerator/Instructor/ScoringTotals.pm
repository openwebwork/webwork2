...
package WeBWorK::ContentGenerator::Instructor::ScoringTotals;
use base qw(WeBWorK::ContentGenerator::Instructor);

=head1 NAME
 
WeBWorK::ContentGenerator::Instructor::ScoringTotals - Generate scoring data files
...
sub initialize {
	...
	if (defined $r->param('scoreSelected')) {
		...
		foreach my $setID (@selected) {
			...
			my $filePath   = "$scoringDir/s${setID}scr.csv";
			$self->writeCSV($filePath, @scoringData);
		}
	#$self->{scoring_file} = $filePath;
	}
}

sub title {
	my ($self, @components) = @_;
	return "Scoring for ".$self->{ce}->{courseName};
}
...
sub body {
	...
	my $scoringDir = $ce->{courseDirs}->{scoring};
	#my $filePath   = $self->{scoring_file};
	# print'<pre>', WeBWorK::Utils::readFile($filePath), '</pre>';
	my @selected = $r->param('selectedSet');
	@selected = (1);
	my $string = '';
	foreach my $setID (@selected) {
