...
async sub lms_roster_sync_handler ($c) {
	...
	my $namesRolesServiceURL = $c->db->getSettingValue('LTINamesRolesServiceURL');
	return (0, $c->maketext('The LTI names/roles service URL is not available.')) unless $namesRolesServiceURL;

	$c->minion->enqueue(lms_roster_sync => [], { notes => { courseID => $ce->{courseName} } });
	return (1, $c->maketext('Roster synchronization queued.'));
}
