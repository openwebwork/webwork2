...
#			"Data",
		...
		foreach my $headerType (@headers) {
			...
			my $editHeaderPage = $urlpath->new(type => 'instructor_problem_editor3_withset_withproblem', args => { courseID => $courseID, setID => $setID, problemID => 0 });
	...
	if (scalar @problemIDList) {
		...
		foreach my $problemID (@problemIDList) {
			...
			my ( $editProblemPage, $editProblemLink, $viewProblemPage,
			     $viewProblemLink );
			if ( $isGatewaySet ) {
				$editProblemPage = $urlpath->new(type =>'instructor_problem_editor3_withset_withproblem', args => { courseID => $courseID, setID => $fullSetID, problemID => $problemID });
