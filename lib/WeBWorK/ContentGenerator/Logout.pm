...
sub pre_header_initialize {
	...
	my $proctorID = defined($r->param("proctor_user")) ? $r->param("proctor_user") : '';
	if ($proctorID) {
		eval { $db->deleteKey("$userID,$proctorID"); };
		if ($@) {
			$keyError .= CGI::p("Error when clearing proctor key: $@");
		}
		# we may also have a proctor key from grading the test
		eval { $db->deleteKey("$userID,$proctorID,g"); };
		if ($@) {
			$keyError .= CGI::p("Error when clearing proctor grading key: $@");
		}
	}
