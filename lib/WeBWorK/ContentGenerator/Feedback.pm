...
sub body {
	...
	if (defined $r->param("sendFeedback")) {
		...
		print $MAIL
			...
			wrap("", "", $r->maketext("This message was automatically generated by the WeBWorK system at [_1], in response to a request from [_2]:[_3].",$systemURL,$remote_host,$remote_port)
			), "\n\n";
		
                print $MAIL $r->maketext("Click this link to see the page from which the user sent feedback:")."\n",
			"$emailableURL\n\n";
		
		if ($feedback) {
			print $MAIL
				$r->maketext("***** The feedback message: *****")."\n\n",
				wrap("", "", $feedback), "\n\n";
		}
		if ($problem and $verbosity >= 1) {
			print $MAIL
				$r->maketext("***** Data about the problem processor: *****")."\n\n",


				$r->maketext("Display Mode:         [_1]",$displayMode)."\n",
				$r->maketext("Show Old Answers:     ") . ($showOldAnswers ? $r->maketext("yes") : $r->maketext("no")) . "\n",
				$r->maketext("Show Correct Answers: ") . ($showCorrectAnswers ? $r->maketext("yes") : $r->maketext("no")) . "\n",
				$r->maketext("Show Hints:           ") . ($showHints ? $r->maketext("yes") : $r->maketext("no")) . "\n",
				$r->maketext("Show Solutions:       ") . ($showSolutions ? $r->maketext("yes") : $r->maketext("no")) . "\n\n",
		}
		...
		if ($problem and $verbosity >= 1) {
			print $MAIL
                                $r->maketext("***** Data about the problem: *****")."\n\n",
				#$problem->toString(), "\n\n";
				$self->format_userproblem($problem), "\n";
		}
		if ($set and $verbosity >= 1) {
			print $MAIL
				$r->maketext("***** Data about the homework set: *****")."\n\n",
				#$set->toString(), "\n\n";
				$self->format_userset($set), "\n";
		}
		if ($ce and $verbosity >= 2) {
			print $MAIL
				$r->maketext("***** Data about the environment: *****")."\n\n",
				Dumper($ce), "\n\n";
		}
