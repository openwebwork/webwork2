...
# $CVSHeader: webwork2/lib/WeBWorK/Utils/CourseManagement.pm,v 1.43 2007/08/13 22:59:59 sh002i Exp $
# 
...
sub checkCourseTables {
	my ($courseName, $dbLayoutName, $ce) = @_;
	my $str='';
	my %both = ();
	my %schema_only = ();
	my %database_only = ();
	...
	foreach my $table (sort keys %$db) {
	    ...
	    my $database_table_exists = ($db->{$table}->can("delete_table")) ? 1:0;
	    if ($database_table_exists ) { # exists means the table could be deleted.
	       $both{$table_name} = checkTableFields($courseName, $dbLayoutName, $ce, $table);
	    } else {
    ...
    my $dbh =$db->{key}->dbh;                        # grab any database handle
	...
	print CGI::p($str);
	my $tables_ok = not (  %schema_only || %database_only ); # count number of extraneous tables; 0 means ok
	return ($tables_ok,\%both, \%schema_only, \%database_only); # table in both schema & database; found in schema only; found in database only
}
...
sub updateCourseTables {
	my ($courseName, $dbLayoutName, $ce, $table_names) = @_;
	my $db = new WeBWorK::DB($ce->{dbLayouts}->{$dbLayoutName});
	#die "Programmers: Pass reference to the array of table names to be updated." unless ref($table_names)=~/ARRAY/;
...
sub checkTableFields {
	my ($courseName, $dbLayoutName, $ce,$table) = @_;
	my $str='&nbsp;&nbsp;';
	my %both = ();
	my %schema_only = ();
	my %database_only = ();
	...
	my @fields = $db->{$table}->{record}->FIELDS;
...
# 		$database_only{$table}=1 unless $exists;
# 	}
# 	return (\%both, \%schema_only, \%database_only); # table in both schema & database; found in schema only; found in database only
    return $str."<br/>";
}
