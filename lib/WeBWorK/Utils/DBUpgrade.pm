...
$DB_VERSIONS[$i]{course_code} = sub {
	...
	$self->dbh->do(
		"ALTER TABLE `${course}_problem_user` ADD UNIQUE KEY (`user_id`(255), `set_id`(255), `problem_id`)");
...
$DB_VERSIONS[ ++$i ]{desc} =
	"updates hide_score and hide_work in set and set_user tables to allow more (and more descriptive) possible values";
$DB_VERSIONS[$i]{course_code} = sub {
	my $course = shift;
	if ($self->sql_table_exists("${course}_set")) {
...
$DB_VERSIONS[ ++$i ]{desc} =
	"updates hide_score and hide_work in set and set_user tables to have more descriptive values, set default values";
$DB_VERSIONS[$i]{course_code} = sub {
	my $course = shift;
	if ($self->sql_table_exists("${course}_set")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set` MODIFY COLUMN `hide_score` ENUM('N','Y','BeforeAnswerDate') DEFAULT 'N'");
		$self->dbh->do(
			"ALTER TABLE `${course}_set` MODIFY COLUMN `hide_work` ENUM('N','Y','BeforeAnswerDate') DEFAULT 'N'");
	}
	if ($self->sql_table_exists("${course}_set_user")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set_user` MODIFY COLUMN `hide_score` ENUM('N','Y','BeforeAnswerDate') DEFAULT 'N'");
		$self->dbh->do(
			"ALTER TABLE `${course}_set_user` MODIFY COLUMN `hide_work` ENUM('N','Y','BeforeAnswerDate') DEFAULT 'N'");
	}
};

$DB_VERSIONS[ ++$i ]{desc} =
	"adds locations, location_addresses, set_locations and set_locations_user tables to database, and add restrict_ip to set and set_user.";
$DB_VERSIONS[$i]{global_code} = sub {
	$self->dbh->do(
		"CREATE TABLE locations (location_id TINYBLOB NOT NULL, description TEXT, PRIMARY KEY (location_id(1000)))"
	);
	$self->dbh->do(
		"CREATE TABLE location_addresses (location_id TINYBLOB NOT NULL, ip_mask TINYBLOB NOT NULL, PRIMARY KEY (location_id(500),ip_mask(500)))"
	);
};
$DB_VERSIONS[$i]{course_code} = sub {
	my $course = shift;
	...
	$self->dbh->do(
		"CREATE TABLE `${course}_set_locations` (set_id TINYBLOB NOT NULL, location_id TINYBLOB NOT NULL, PRIMARY KEY (set_id(500),location_id(500)))"
	);
	$self->dbh->do(
		"CREATE TABLE `${course}_set_locations_user` (set_id TINYBLOB NOT NULL, user_id TINYBLOB NOT NULL, location_id TINYBLOB NOT NULL, PRIMARY KEY (set_id(300),user_id(300),location_id(300)))"
	);

	if ($self->sql_table_exists("${course}_set")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set` ADD COLUMN `restrict_ip` enum('No','RestrictTo','DenyFrom') DEFAULT 'No'");
	}
	if ($self->sql_table_exists("${course}_set_user")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set_user` ADD COLUMN `restrict_ip` enum('No','RestrictTo','DenyFrom')");
	}
};
...
$DB_VERSIONS[$i]{course_code} = sub {
	...
	if ($self->sql_table_exists("${course}_set_user")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set_user` MODIFY COLUMN `hide_score` ENUM('N','Y','BeforeAnswerDate')");
		$self->dbh->do(
			"ALTER TABLE `${course}_set_user` MODIFY COLUMN `hide_work` ENUM('N','Y','BeforeAnswerDate')");
	}
};
...
$DB_VERSIONS[$i]{course_code} = sub {
	...
	if ($self->sql_table_exists("${course}_set")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set` ADD COLUMN `relax_restrict_ip` ENUM('No','AfterAnswerDate','AfterVersionAnswerDate') DEFAULT 'No'"
		);
		$self->dbh->do("ALTER TABLE `${course}_set` ADD COLUMN `hide_score_by_problem` ENUM('N','Y') DEFAULT 'N'");
	}
	if ($self->sql_table_exists("${course}_set_user")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set_user` ADD COLUMN `relax_restrict_ip` ENUM('No','AfterAnswerDate','AfterVersionAnswerDate')"
		);
...
$DB_VERSIONS[ ++$i ]{desc} =
	"adds set and set_user fields to allow set-level proctor, updates permissions to allow finer-grained regulation of proctoring.";
$DB_VERSIONS[$i]{course_code} = sub {
	my $course = shift;
	if ($self->sql_table_exists("${course}_permission")) {
	...
	if ($self->sql_table_exists("${course}_set")) {
		$self->dbh->do(
			"ALTER TABLE `${course}_set` ADD COLUMN `restricted_login_proctor` ENUM('No','Yes') DEFAULT 'No'");
	}
	if ($self->sql_table_exists("${course}_set_user")) {
...
sub check_db_version_format {
	...
	if (not defined $db_version) {
		...
		return
			"'db_version' is set to 0, which is reserved to indicate a pre-automatic-upgrade version. $vers_value_should_be";
	} else {
...
sub do_upgrade {
	...
	if ($system_db_version == DB_TABLE_MISSING) {
		...
		die
			"This database's system db_version value is $system_db_version, but the current database version is only $THIS_DB_VERSION. This database was probably used with a newer version of WeBWorK. Can't continue.\n";
	}
	...
	foreach my $course (@ww_courses) {
		...
		if ($system_db_version < $FIRST_COURSE_DB_VERSION) {
			...
			if ($course_db_version == DB_TABLE_MISSING) {
				...
				$self->verbose(
					"Course '$course' has no db_version of it's own, assuming system db_version $system_db_version.\n");
				...
				if ($course_db_version == DB_RECORD_MISSING) {
					...
					warn
						"This course's version is $course_db_version, which is before per-course versioning was introduced."
			...
			if ($course_db_version == DB_TABLE_MISSING) {
				...
				if ($course_db_version == DB_RECORD_MISSING) {
					...
					warn "$error\n";
					warn
						"The course '$course' has an invalid value in the 'db_version' record in its '${course}_setting' table,"
						. " so we can't determine the version. This course will be ignored."
						. " If you know the version of this course,"
			...
			if ($result == ERROR) {
				...
				if ($self->ask_permission("Upgrade the remaining courses to version $vers?", 0)) {
					...
					if ($self->ask_permission(
						"Update the stored db_version for the courses we're skipping, as if we had upgraded them?",
						1
					))
					{
						$do_upgrade = 0;
...
sub upgrade_course_to_version {
	...
	if (not defined $course_vers) {
		...
		warn
			"Course '$course' at version $course_vers, which is too old to upgrade to $vers. This should never happen. Not upgrading.\n";
		return SKIPPED;
	}
