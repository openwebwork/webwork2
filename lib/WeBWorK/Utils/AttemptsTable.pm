...
use Class::Accessor::Lite (
    new => 0,
    rw  => [qw(correct_ids incorrect_ids showMessages summary)],
    ro  => [
        qw(answers answerOrder answersSubmitted displayMode imgGen maketext_coderef
          showAnswerNumbers showAttemptAnswers showAttemptPreviews showAttemptResults
...
sub new {
	...
	my $self = {
	    ...
	    maketext_coderef    => $options{maketext}//sub {return @_},# pointer to the maketext subroutine
...
#	return 'xXx'.&{$self->{maketext}}(@_).'xXx';
	return &{$self->{maketext_coderef}}(@_);
}
sub formatAnswerRow {
	my $self          = shift;
	my $rh_answer     = shift;
