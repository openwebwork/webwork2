...
sub render {
	...
	if($self->{count}==0) {return ;}  # Don't run latex if there are no images

	my $refreshMe = 0;
	if (defined($opts{refresh}) and
			(($opts{refresh} eq "yes") or ($opts{refresh} == 1))) {
		$refreshMe = 1;
	}
...
#	$refreshMe = 1;  # Uncomment for testing
	...
	if ($refreshMe or not -e $tempFile
            or (stat $sourcePath)[9] > (stat $tempFile)[9]) {
