...
package WeBWorK::DB::Schema::Moodle::Password;
use base qw(WeBWorK::DB::Schema);

=head1 NAME

WeBWorK::DB::Schema::Moodle::Password - support access to Moodle's password store.
...
use constant TABLES => qw(password);
...
################################################################################
# constructor for Moodle::Password-specific behavior
################################################################################
...
sub count {
	...
	croak "Too many keyparts for table password. Need at most @keynames"
...
sub gets($@) {
	...
	foreach my $keypartsRef(@keypartsRefList) {
		...
		if( not defined $keyparts[0] ) {
			croak "wrong number of keyparts for table key";
		}
		
		my $qry = "SELECT username, password FROM `$table` WHERE username=?";
		my @qryArgs = ($keyparts[0]);
		$self->debug("SQL-gets: $qry\n");
