...
sub new {
	...
	$self->{userids_subquery} = [ $self->_userids_subquery($self->{params}{courseName}) ];
...
sub _all_course_userids_subquery {
	my ($self, $type) = @_;
	my $stmt = $self->sql->select(["user_$type"], ["user_$type.userid"]);
}
...
sub _conv_fields_ww2mdl {
	...
	foreach my $field (@$fields) {
		...
		if ($field eq "status") {
			...
			my $status_d = $self->dbh->quote($self->{params}{statusDeleted});
			my $status_c = $self->dbh->quote($self->{params}{statusNotDeleted});
...
sub craptastic {
	...
	if ($need_sec_rec) {
		# add tpush @having,o fields list if they're not there already
		...
		push @bind_vals, $self->{params}{courseName};
...
sub count_where {
	my ($self, $where) = @_;
	($where, my (%params)) = $self->conv_where($where);
	# params can contain:
	# select_on_status
	# select_on_section
	# select_on_recitation
	croak "section/recitation/status not supported yet"
		if exists $params{select_on_status}
		or exists $params(select_on_section}
		or exists $params{select_on_recitation};
	
	# get userids subquery
	my ($sub_stmt, @sub_bind_vals) = @{$self->{userids_subquery}};
	print "sub_stmt=$sub_stmt\n";
	
	# generate the main WHERE clause
	my ($stmt, @bind_vals) = $self->sql->where($where);
	print "stmt=$stmt\n";
	
	# prepend the "id IN ( subquery )" part
	my $idfield = $self->sql->_quote("id");
	$stmt =~ s/\bWHERE\b/WHERE $idfield IN ( $sub_stmt )/
		or $stmt = " WHERE $idfield IN ( $sub_stmt ) $stmt";
	
	# prepend the bind values from the subquery
	unshift @bind_vals, @sub_bind_vals;
	
	# prepend the head of the select statement
	(substr($stmt, 0, 0)) = $self->sql->select("user", "COUNT(*)");
