...
package WeBWorK::File::BBClasslist;
...
our @EXPORT = qw/parse_BBclasslist write_BBclasslist/;

sub parse_BBclasslist{
	#my ($file, $emailExt) = (@_);		#my($file) references a local version of $file.
	my $file = shift;
	my $emailExt = shift;
	my $count = 0;
	...
	while (<$fh>) {			# <$fh> - reads in every file.
		...
		my @fields = ( '',$tempfields[0],$tempfields[1],'','','','',"$tempfields[2]"."$emailExt",$tempfields[2],$tempfields[2],0 );
