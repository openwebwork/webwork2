...
sub new {
	...
	#initialize
#	our $GENERIC_ERROR_MESSAGE = $r->maketext(
#	"Your authentication failed.  Please return to "
#	. "your Course Management System (e.g., Oncourse, Moodle, "
#	. "Blackboard, Canvas, Sakai, etc.)  and login again.");
#	our $GENERIC_MISSING_USER_ID_ERROR_MESSAGE = $r->maketext(
#	"Your authentication failed.  Please return to "
#	. "your Course Management System (e.g., Oncourse, Moodle, "
#	. "Blackboard, Canvas, Sakai, etc.)  and login again.");
#	our $GENERIC_DENIED_LOGIN_ERROR_MESSAGE = $r->maketext(
#	"You are not permitted to login into this site at this time. "
#	. "Please speak with your instructor.");
#	our $GENERIC_UNKNOWN_USER_ERROR_MESSAGE = $r->maketext(
#	"This username does not appear on the roster for this WeBWorK site.") ;
#	our $GENERIC_UNKNOWN_INSTRUCTOR_ERROR_MESSAGE = $r -> maketext(
#	"You have attemped to access this site as an instructor without prior authorization.");
	bless $self, $class;
	return $self;
}
...
#   # Response: The local copy is modifiable but the underlying C table is not.
#   #           See the perl documentation on Apache2::Request at the end of the subsection
#   #           on "param".
#
...
# sub create_session {
# 	my $self = shift;
# 	my ($r, $userID, $newKey) = map {$self -> {$_};}
# 		('r', 'user_id', 'session_key');
# 	my ($ce, $db) = map {$r -> $_ ;} ('ce', 'db');
# 	
# 	my $timestamp = time;
# 	unless ($newKey) {
...
# 	debug("Session Key constructed.");
# 	debug("Mark L");
# 	eval { $db->deleteKey($userID) };
# 	debug("return from deleteKey = |$@|.");
# 	debug("Mark M");
# 	debug("Process has returned from calling deleteKey.");
#   if ($db->{key}->exists($userID)) 
# 	{
# 		debug("putting key");
# 		$db -> putKey($Key);
# 	}
...
# 	}
# 	debug("Mark N");
# 	debug("Key as been added to database.");
# 	$self->sendCookie($userID, $newKey);
# 	return $newKey;
# }
# 
# # Must kill any existing session for this user
# # by killing any cookie and any session key.
# sub check_session {
...
# 	my ($cookie_user, $cookie_key) = $self->fetchCookie();
# 
# 	if (defined($cookie_user) && $userID eq $cookie_user) {
# 		$self -> killCookie(); 
# 	}
...
##		-expires => $expires,              # Want a session cookie, not a persistent cookie
##		-domain  => $r->hostname,          # if $r->hostname = "localhost" or "127.0.0.1", then this must be omitted. See Below.
#		-path    => $ce->{webworkURLRoot},
#		-secure  => 0,
#	);
#
#	if ($r->hostname ne "localhost" && $r->hostname ne "127.0.0.1") {
#		$cookie -> domain($r->hostname);
#	}
#	
...
#		-name => "WeBWorKCourseAuthen.$courseID",
#		-value => "\t",
##		-expires => $expires,              # Session Cookies only.
##		-domain  => $r->hostname,          # if $r->hostname = "localhost" or "127.0.0.1", then this must be omitted. See Below.
#		-path => $ce->{webworkURLRoot},
#		-secure => 0,
#	);
