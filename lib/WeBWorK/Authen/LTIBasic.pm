...
our $GENERIC_DENIED_LOGIN_ERROR_MESSAGE =
	"You are not permitted to login into this site at this time. " . "Please speak with your instructor.";
our $GENERIC_UNKNOWN_USER_ERROR_MESSAGE = "This username does not appear on the roster for this WeBWorK site.";
...
sub get_credentials {
	...
	if (defined($self->{user_id}) && $self->{user_id} ne "") {
		...
		if (defined($ce->{LTI}{v1p1}{preferred_source_of_student_id})
			&& defined($c->param($ce->{LTI}{v1p1}{preferred_source_of_student_id})))
		{
		...
		if (!defined($self->{user_id})) {
			croak
				"LTIBasic was unable to create a username from the data provided with the current settings. Set \$debug_lti_parameters=1 in authen_LTI.conf to debug";
		}
...
sub authenticate {
	...
	$path = $ce->{LTI}{v1p1}{BasicToThisSiteURL} ? $ce->{LTI}{v1p1}{BasicToThisSiteURL} : $path;
	...
	if ($@) {
		...
		if (!$request->verify && !$altrequest->verify) {
			...
			my $LTI_webwork_permissionLevel =
				$ce->{userRoles}->{ $ce->{LTI}{v1p1}{LMSrolesToWeBWorKroles}->{ $LTIroles[0] } };
			if ($nr > 1) {
				for (my $j = 1; $j < $nr; $j++) {
			...
			if (!$db->existsUser($userID)) {    # New User. Create User record
				warn "New user: $userID -- requested permission level is $LTI_webwork_permissionLevel.
				      Only new users with permission levels less than or equal to 'ta = 5' can be created."
					if ($ce->{debug_lti_parameters});
				...
				$newUser->status("C");
				$newUser->section(($LTI_webwork_permissionLevel > $ce->{userRoles}->{"student"}) ? "Admin"
					: (defined($self->{section})) ? $self->{section}
					:                               "");
				...
				foreach $globalSet (@GlobalSets) {
					if (defined($globalSet)) {
						if (defined($ce->{"adjustDueDatesForLateAdds"})
							and $ce->{"adjustDueDatesForLateAdds"}
							and $globalSet->open_date < $open_cut
							and $globalSet->due_date < $due_cut)
						{
			...
			} else {    # Existing user.  Possibly modify demographic information and permission level.
				my $user            = $db->getUser($userID);
				my $permissionLevel = $db->getPermissionLevel($userID);
				if (
					($user->last_name() eq "Teacher" and $user->first_name() eq "The")
				...
				{    #This is the instructor of record or an administrator.  No changes permitted via LTI.
					...
					if (defined($self->{"recitation"})
						and defined($user->recitation)
...
#################################################################
					{
						...
						warn "Setting permission level for $userID to $LTI_webwork_permissionLevel"
							if ($ce->{debug_lti_parameters});
					}
	...
	$self->{error} = $c->maketext($GENERIC_ERROR_MESSAGE, $ce->{LTI}{ $ce->{LTIVersion} }{LMS_name});
	return (0);
}
