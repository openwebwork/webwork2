...
# $CVSHeader: /webwork/cvs/system/webwork2/lib/WeBWorK/Authen/LTIAdvanced.pm,v 1.1 2012/05/17 18:50:11 wheeler Exp $
# 
...
# 		}
		...
		if (!defined($self->{user_id})) {
			croak "LTIAdvanced was unable to create a username from the user_id or from the mail address.
...
EOS
			...
			my $gradeRequest = Net::OAuth->request("consumer")->new(request_url => $r->param('lis_outcome_service_url'),
										     request_method => "POST",
										     consumer_secret => $ce->{LTIBasicConsumerSecret},
...
######## Changed due to Instructor roles passed from Sakai/Oncourse to LTIAdvanced ######
...
#################################################################
					...
					$self->write_log_entry("\n\n\nPermission level for user $userID set to $LTI_webwork_permissionLevel via LTIAdvanced login");
