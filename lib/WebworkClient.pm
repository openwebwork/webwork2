...
sub xmlrpcCall {
	...
	$transporter->transport->ssl_opts(
		verify_hostname => 0,
	...
	if ($UNIT_TESTS_ON) {
		...
		print STDERR "\tWebworkClient.pm " . __LINE__ . " full xmlrpcCall path ",
			($self->site_url) . '/' . REQUEST_URI, "\n";
		print STDERR "\tWebworkClient.pm " . __LINE__ . " xmlrpcCall issued with command $command\n";
		print STDERR "\tWebworkClient.pm " . __LINE__ . " input is: ",
			join(" ", map { $_ // '--' } %{ $self->request_object }), "\n";
		print STDERR "\tWebworkClient.pm " . __LINE__
			. " xmlrpcCall $command initiated webwork webservice object $requestResult\n";
	}
	...
	if ($@) {
		...
		print CGI::p("Errors:", CGI::br(), CGI::blockquote({ style => "color:red" }, CGI::code($@)),
			CGI::br(), "End Errors");
	}

	if (not ref($result)) {
		my $error_string = "xmlrpcCall to $command returned no result for " . ($self->{sourceFilePath} // '') . "\n";
...
sub jsXmlrpcCall {
	...
	my $requestResult = $transporter->proxy(($self->site_url) . '/' . REQUEST_URI);
	$transporter->transport->ssl_opts(
		verify_hostname => 0,
