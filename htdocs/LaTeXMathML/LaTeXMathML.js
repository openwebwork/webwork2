...
This is a modified verson of Douglas Woodall's LaTeXMathML.js file for
use with WeBWorK.  I have marked the changes with "DPVC" comments.
These include adding a few missing commands that we needed, and making
it look for $$..$$ rather than just $...$ so as to avoid matching
dollar signs used within the problem text itself.  It is also modified
to not reinitialize itself if loaded a second time (as it will be in
the Library Browser).
...
{input:"\\colon",	tag:"mo", output:":",	   ttype:CONST}, // DPVC
...
{input:"\\rm",     tag:"mstyle", output:"\\mathrm", ttype: DEFINITION},             // DPVC
{input:"\\bf",     tag:"mstyle", atname:"mathvariant", atval:"bold", ttype:UNARY},  // DPVC
{input:"\\it",     tag:"mstyle", atname:"mathvariant", atval:"italic", ttype:UNARY} // DPVC
];
  ...
  if (n.childNodes.length == 0) {
   if ((n.nodeType!=8 || linebreaks) &&
    ...
    if (!(str == null)) {
      ...
      str = str.replace(/\s*\r\n/g," ");
// DELIMITERS:
      mtch = (str.indexOf("\$\$")==-1 ? false : true);   //  DPVC use $$ instead of $
//      str = str.replace(/([^\\])\$/g,"$1 \$");         //  DPVC no need for these with $$
//      str = str.replace(/^\$/," \$");	// in case \$ at start of string
      arr = str.split("\$\$");                           // DPVC split on $$ 
//      for (i=0; i<arr.length; i++)             // DPVC (use \$ macro above instead)
//	arr[i]=arr[i].replace(/\\\$/g,"\$");     // DPVC
...
// DELIMITERS:
    if (st==null || st.indexOf("\$\$")!=-1)    // DPVC (use $$ not $ for delimiter)
