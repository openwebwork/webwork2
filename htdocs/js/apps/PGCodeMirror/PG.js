...
//			'$b'				:    5,
		...
		function tokenPerl(stream,state){
			...
			if(stream.match(/^BEGIN_TEXT\s*$/)) {
				state.tokenize = function(stream,state) {
					return tokenEV3(stream,state,"END_TEXT","meta");
				}
				return PGkeyword;
			}
			if(stream.match(/^BEGIN_HINT\s*$/)) {
				state.tokenize = function(stream,state) {
					return tokenEV3(stream,state,"END_HINT","meta");
				}
				return PGkeyword;
			}
			if(stream.match(/^BEGIN_SOLUTION\s*$/)) {
				state.tokenize = function(stream,state) {
					return tokenEV3(stream,state,"END_SOLUTION","meta");
				}
				return PGkeyword;
			}
			if(stream.match(/^BEGIN_PGML\s*$/)) {
				state.tokenize = function(stream,state) {
					return tokenPGML(stream,state,"END_PGML","meta");
				}
				return PGkeyword;
			}
			if(stream.match(/^BEGIN_PGML_HINT\s*$/)) {
				state.tokenize = function(stream,state) {
					return tokenPGML(stream,state,"END_PGML_HINT","meta");
				}
				return PGkeyword;
			}
			if(stream.match(/^BEGIN_PGML_SOLUTION\s*$/)) {
				state.tokenize = function(stream,state) {
					return tokenPGML(stream,state,"END_PGML_SOLUTION","meta");
				}
				return PGkeyword;
			}
