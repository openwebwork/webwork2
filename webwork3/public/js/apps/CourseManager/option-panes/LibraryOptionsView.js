define(['backbone','views/SidePane', 'config'],function(Backbone,SidePane,config){
    ...
    initialize: function(options){
        ...
        this.problemSets.on("add",function(){
            self.stickit();
        })
    },
    render: function(){
    ...
    events: {
        ...
        "change .select-target-option": function (evt) {
            this.$(".goto-problem-set-button").attr("disabled",false);
        ...
        "click .goto-problem-set-button": function (evt) {this.trigger("goto-problem-set",this.model.get("target_set"))}
    }
});
