...
sub Parser::BOP::add::D {
	my $self = shift;
	my $x    = shift;
	$self = Parser::BOP->new($self->{equation}, $self->{bop}, $self->{lop}->D($x), $self->{rop}->D($x));
	return $self->reduce;
}
...
sub Parser::BOP::multiply::D {
	...
	$self = Parser::BOP->new(
		$equation, '+',
		Parser::BOP->new($equation, $self->{bop}, $self->{lop}->D($x),           $self->{rop}->copy($equation)),
		Parser::BOP->new($equation, $self->{bop}, $self->{lop}->copy($equation), $self->{rop}->D($x))
	);
	return $self->reduce;
}
...
sub Parser::BOP::divide::D {
	...
	$self = Parser::BOP->new(
		...
		Parser::BOP->new(
			$equation, '-',
			Parser::BOP->new($equation, '*', $self->{lop}->D($x),           $self->{rop}->copy($equation)),
			Parser::BOP->new($equation, '*', $self->{lop}->copy($equation), $self->{rop}->D($x))
		),
		Parser::BOP->new($equation, '^', $self->{rop}, Parser::Number->new($equation, 2))
	);
	return $self->reduce;
}
...
sub Parser::BOP::power::D {
	...
	if (defined($vars->{$x})) {
		...
		$self = Parser::BOP->new(
			$equation, '*',
			Parser::Function->new($equation, 'log', [ $self->{lop}->copy($equation) ], 0),
			Parser::BOP->new($equation, '*', $self->copy($equation), $self->{rop}->D($x))
		);
	} else {
		$self = Parser::BOP->new(
			$equation,
...
sub Parser::Function::trig::D_cos {
	...
	my $x        = shift;
	my $equation = $self->{equation};
	return Parser::UOP->new($equation, 'u-', Parser::Function->new($equation, 'sin', [$x]));
}
		...
		Parser::Function->new(
			...
			[
				Parser::BOP->new(
					$equation, '-',
					Parser::Number->new($equation, 1),
					Parser::BOP->new($equation, '^', $x, Parser::Number->new($equation, 2))
				)
			]
		)
	);
...
sub Parser::Function::numeric::D_log10 {
	...
	return Parser::BOP->new(
		$equation, '/',
		Parser::Number->new($equation, 1),
		Parser::BOP->new($equation, '*', Parser::Number->new($equation, CORE::log(10)), $x)
	);
}
...
sub Parser::Function::numeric::D_sqrt {
	...
	return Parser::BOP->new(
		$equation, '/',
		Parser::Number->new($equation, 1),
		Parser::BOP->new($equation, '*', Parser::Number->new($equation, 2), $self->copy)
	);
}
